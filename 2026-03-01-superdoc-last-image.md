...
This would explain the exact symptom perfectly: one image, everywhere, always the last one.

That said, this is a hypothesis, not a confirmed fix. The next step is to patch `docx-templates` output to inject unique sequential IDs, do a clean server restart, and verify the result properly. That last part matters — a failed test earlier in this session turned out to be against old compiled code because the server hadn't been restarted after a rebuild. It's a basic mistake but it cost hours, so worth flagging.
...
| Patch to unique IDs                    | Not fully verified yet — requires clean test |
...
Word tolerates many OOXML violations silently. Editors and converters are stricter. Never assume Word rendering equals correctness — it is one of the most forgiving parsers in the ecosystem.

### 2. Always inspect DOCX internally

DOCX debugging becomes much faster once you treat it as a zip file. Unzip it, inspect the XML, search for patterns, verify structure. This approach surfaced the duplicate ID issue in minutes once I started looking — no console log would have caught it.
...
Sharing this in case someone else hits the same wall while building document editors. If you've encountered similar behaviour with DOCX image rendering — or know for certain whether SuperDoc uses `pic:cNvPr id` for node deduplication — I'd genuinely like to hear what you found.
